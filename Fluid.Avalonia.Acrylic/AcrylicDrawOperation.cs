...
{
    internal class AcrylicDrawOperation : ICustomDrawOperation
    {
        ...
        private readonly AcrylicDrawParameters _parameters;
        private readonly AcrylicBackdropSnapshot? _backdropSnapshot;
        private readonly AcrylicDrawPass _pass;

        public AcrylicDrawOperation(
            Rect bounds,
            AcrylicDrawParameters parameters,
            AcrylicBackdropSnapshot? snapshot,
            AcrylicDrawPass pass)
        {
            _bounds = bounds;
        ...
        public void Render(ImmediateDrawingContext context)
        {
            ...
            switch (_pass)
            {
                case AcrylicDrawPass.Lens:
                    RenderLens(canvas, lease.GrContext);
                    break;
                case AcrylicDrawPass.InteractiveHighlight:
        ...
        private static void LoadShaders()
        {
            ...
            s_lensEffect = LoadRuntimeEffect("avares://Fluid.Avalonia.Acrylic/Assets/Shaders/AcrylicShader.sksl");
            s_highlightEffect = LoadRuntimeEffect("avares://Fluid.Avalonia.Acrylic/Assets/Shaders/AcrylicHighlight.sksl");
            s_gammaEffect = LoadRuntimeEffect("avares://Fluid.Avalonia.Acrylic/Assets/Shaders/AcrylicGamma.sksl");
            s_interactiveHighlightEffect = LoadRuntimeEffect("avares://Fluid.Avalonia.Acrylic/Assets/Shaders/AcrylicInteractiveHighlight.sksl");
            s_progressiveMaskEffect = LoadRuntimeEffect("avares://Fluid.Avalonia.Acrylic/Assets/Shaders/AcrylicProgressiveMask.sksl");
            s_backdropTransformEffect = LoadRuntimeEffect("avares://Fluid.Avalonia.Acrylic/Assets/Shaders/AcrylicBackdropTransform.sksl");
        }
        ...
        private static SKRuntimeEffect? LoadRuntimeEffect(string assetUriString)
        {
            try
            {
                ...
                if (effect == null)
                    Console.WriteLine($"[Acrylic] Failed to create SKRuntimeEffect ({assetUriString}): {errorText}");

                return effect;
            }
            catch (Exception ex)
            {
                Console.WriteLine($"[Acrylic] Exception while loading shader ({assetUriString}): {ex.Message}");
                return null;
            }
        }
        ...
        private void RenderLens(SKCanvas canvas, GRContext? grContext)
        {
            ...
            AcrylicBackdropSnapshot.FilteredResult filtered = GetOrCreateFilteredBackdrop(
                _backdropSnapshot,
                _parameters,
                grContext,
                currentTransform,
            ...
            SKSize localSize)
        {
            ...
            if (isIdentity)
            {
                AcrylicDiagnostics.RecordFilterIdentityHit();
                return new AcrylicBackdropSnapshot.FilteredResult(snapshot.Image, snapshot.OriginInPixels);
            }
            ...
            AcrylicBackdropSnapshot.FilteredKey key = new(
            ...
            if (snapshot.TryGetFiltered(key, out AcrylicBackdropSnapshot.FilteredResult cached))
            {
                AcrylicDiagnostics.RecordFilterCacheHit();
                return cached;
            }

            AcrylicDiagnostics.RecordFilterCacheMiss();

            AcrylicBackdropSnapshot.FilteredResult filtered = CreateFilteredBackdrop(snapshot, cropRect, brightness, contrast, saturation, exposureEv, opacity, blurSigmaPx, grContext)
                                                                  ?? new AcrylicBackdropSnapshot.FilteredResult(snapshot.Image, snapshot.OriginInPixels);
        ...
        private static AcrylicBackdropSnapshot.FilteredResult? CreateFilteredBackdrop(
            AcrylicBackdropSnapshot snapshot,
            SKRectI cropRect,
            float brightness,
            float contrast,
            ...
            GRContext? grContext)
        {
            ...
            try
            {
                ...
                if (surface is null)
                {
                    AcrylicDiagnostics.RecordFilterSurfaceFailure();
                    return null;
                }
        ...
        private static double CalculateFilterMarginPx(AcrylicDrawParameters parameters, SKSize localSize, double scaling)
        {
        ...
        private static SKSurface? CreateFilterSurface(SKImageInfo info, GRContext? grContext)
        {
            if (grContext is not null)
            {
                ...
                {
                    AcrylicDiagnostics.RecordFilterGpuSurface();
                    return gpuSurface;
                }
            }
            ...
            if (cpuSurface is not null)
                AcrylicDiagnostics.RecordFilterCpuSurface();
